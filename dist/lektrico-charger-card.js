...
`}
    ...
    `}_renderQuickActions(t){const e=this._stateObj("charge_start"),i=this._stateObj("charge_stop"),s=this._stateObj("authentication"),r=this._stateObj("lock"),o=[];return e&&o.push(D`
      ...
      `),o.length?D`<div class="quick-actions">${o}</div>`:V}_renderSections(t=!1){const e=[];if(t||!1===this._config.show_parameters||e.push({id:"parameters",icon:"mdi:speedometer",title:this._config.section_titles?.parameters||this._t("parameters"),body:()=>this._renderParameters()}),t||!1===this._config.show_info||e.push({id:"info",icon:"mdi:information-outline",title:this._config.section_titles?.info||this._t("info"),body:()=>this._renderInfoList()}),!1!==this._config.show_actions){const t=this._renderActionsGrid();t!==V&&e.push({id:"actions",icon:"mdi:gesture-tap-button",title:this._config.section_titles?.actions||this._t("actions"),body:()=>t})}return e.length?D`
      <div class="sections">
        ${e.map(t=>D`
            <div class="section">
              <button
    ...
    `:V}_renderSlider(t,e,i,s){const r=this._stateObj(t);if(!r)return V;const o=r.attributes,n=Number(o.min??0),a=Number(o.max??100),c=Number(o.step??1),l=Number(r.state),d=this._pendingSlider[t]??(Number.isNaN(l)?n:l),h=s||o.unit_of_measurement||"",p=(i||[]).filter(t=>t>=n&&t<=a);return D`
