...
`}
    ...
    `}_renderSlider(t,e,i,s){const r=this._stateObj(t);if(!r)return K;const o=r.attributes,n=Number(o.min??0),a=Number(o.max??100),c=Number(o.step??1),l=Number(r.state),d=this._pendingSlider[t]??(Number.isNaN(l)?n:l),h=s||o.unit_of_measurement||"",p=(i||[]).filter(t=>t>=n&&t<=a);return D`
    ...
    `}_renderParameters(){const t=this._stateObj("authentication"),e=this._stateObj("lock");return D`
      ${this._renderSlider("dynamic_limit","dynamic_limit",this._config.current_presets??Ct,"A")}
      ${this._renderSlider("led_brightness","led_brightness",this._config.brightness_presets??Ot,"%")}
    ...
    `}_renderStats(t,e){const i=[{entity:"session_energy",label:this._t("energy")},{entity:"charging_time",label:this._t("charging_time")},{entity:"temperature",label:this._t("temperature")}],s=this._config.stats||{};let r=Array.isArray(s)?s:s[t]||s.default||i;return"error"===t&&e.length&&!s.error&&(r=e.map(t=>({entity:t.entity_id,label:this._errorName(this._errorKeyOf(t.entity_id),t),value:"⚠"}))),D`
      <div class="stats">
        ${r.map(t=>{const e="string"==typeof t?{entity:t}:t,i=this._stateObj(e.entity);if(!i)return K;const s=e.label||(ft.en.ui[e.entity]?this._t(e.entity):i.attributes.friendly_name);return D`
            <div
              class="stat"
              @click=${()=>this._moreInfo(i.entity_id)}
            >
